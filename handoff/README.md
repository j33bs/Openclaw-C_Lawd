...
- Tracked wrapper surface: `npm run interbeing:emit -- --task-id task-demo-001 --prompt "..."`
...
- `npm run interbeing:emit` writes directly to a local intake only when `--intake-dir`, `OPENCLAW_DALI_INTAKE_DIR`, or the sibling Dali intake `../openclaw-dali/handoff/incoming/dali/` exists.
- When no local intake lane exists, `npm run interbeing:emit` falls through to the existing Dali ssh/scp lane using `OPENCLAW_INTERBEING_DALI_*` config or `--remote-host`, `--remote-user`, `--remote-port`, and `--remote-dir`.
- Override the local direct-write lane with `OPENCLAW_DALI_INTAKE_DIR=/custom/path/handoff/incoming/dali npm run interbeing:emit -- ...` or `npm run interbeing:emit -- --intake-dir /custom/path/handoff/incoming/dali ...`.
...
- The tracked wrapper exposes the accepted planner/executor lineage subset behind `--include-local-dispatch`, keeps wrapper-only evidence under `workspace/audit/_evidence/interbeing-emitter-v0/`, and preserves `child_task_id` there instead of emitting it into the canonical envelope.
- The tracked wrapper now also passes through accepted adapter-local `payload.local_dispatch.task_contract` fields via `--task-class`, repeatable `--acceptance-criterion`, `--review-mode`, `--worker-limit`, and `--execution-notes`.
