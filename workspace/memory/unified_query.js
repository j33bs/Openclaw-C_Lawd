...
function makeCorrespondenceStoreAdapter(opts = {}) {
  ...
  return {
    ...
    async query(params = {}) {
      ...
      const baseUrl =
        opts.baseUrl || process.env.CORRESPONDENCE_STORE_BASE_URL || "http://127.0.0.1:8765";
      ...
      try {
        ...
        return payload.map((row) => ({
          ...
          text: row.body || "",
          refs: [
            row.canonical_section_number !== undefined
              ? `canonical:${row.canonical_section_number}`
              : null,
          ].filter(Boolean),
        }));
      } catch {
...
function createDefaultAdapters(opts = {}) {
  return [
    ...
    makeUserProfileAdapter(opts.profile || {}),
  ];
}
